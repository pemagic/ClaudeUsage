...
Claude Code 的各个付费订阅计划（Pro / Team / Max / Enterprise）都有会话（5小时）和周（7天）用量限制，但没有方便的方式查看消耗了多少——你得打开 CLI 手动运行 `/usage`。ClaudeUsage 安静地待在菜单栏，一眼就能看到百分比，还有消耗速度预测、重置倒计时和费用追踪。
...
- **双尺寸** — 小号（紧凑 180×190 方块）或中号（340×160 完整信息）
- **可拖拽** — 拖到任意位置，重启后自动记住
- **零额外开销** — 与菜单栏共享同一数据源，无需额外轮询或进程间通信

### 智能空闲检测
- 每 15 秒通过 IOKit `HIDIdleTime` 检测系统空闲时间
- 达到空闲阈值后暂停所有轮询，菜单栏显示 ⏸，弹窗和小组件保留缓存数据并标注睡眠状态
- 用户回来后自动恢复并立即刷新
...
- Claude Code 付费订阅计划（Pro / Team / Max / Enterprise 等有用量限制的计划）
